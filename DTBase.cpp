    ...
    while(q!="STOP")
    {
        if(q.size()>5 && q.substr(0,5)=="TABLE") //Queries for table creation and deletion
        {
            ...
            else if(v[1]=="CREATE")
            {
                if(v.size()<4)
                {
                    cerr<<"Invalid query. Column names (OR) Table name missing"<<endl;
                    goto end;
                }
        ...
        else if(q.find("UNION")!=string::npos) //Union type queries 
        {
            ...
            if (find(table_name.begin(), table_name.end(), t_name1) == table_name.end())
            {
                cerr << "Table "<<t_name1<<" Not present" << endl;
                goto end;
            }

            if (find(table_name.begin(), table_name.end(), t_name2) == table_name.end())
            {
                cerr << "Table "<<t_name2<<" Not present" << endl;
                goto end;
            }
            ...
            {
                cerr<<"Invalid statement for union"<<endl;
                goto end;
            }
        ...
        else  // For handle Select,Update,Delete,Insert type queries.
        {
            ...
            if(pos==-1)
            {
                cerr<<"Invalid Query. Table name is not given"<<endl;
                goto end;
            }
            ...
            if (find(table_name.begin(), table_name.end(), t_name) == table_name.end())
            {
                cerr << "Table "<<t_name<<" Not present" << endl;
                goto end;
            }
